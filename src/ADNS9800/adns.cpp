...
//     _selectedFlag = 1;
//     delayNanoseconds(ADNS_DELAYNANOS_NCS_SCLKACTIVE);
//   }
// }
...
void ADNS::select() {
  if (_selectedFlag == false) {
    SPI.beginTransaction(SPISettings(ADNS_SPI_MAX_SPEED, ADNS_SPI_BIT_ORDER, ADNS_SPI_DATA_MODE));
    digitalWriteFast(_chipSelectPin, LOW); // Use Teensy's optimized digitalWriteFast
    delayNanoseconds(ADNS_DELAYNANOS_NCS_SCLKACTIVE); // Adjust if needed (e.g., 50ns)
    _selectedFlag = true;
  }
}
...
//   delayNanoseconds(ADNS_DELAYNANOS_NCSINACTIVE_POST_READ);
//   deselect();
...
uint8_t ADNS::readRegister(const RegisterAddress address) {
  select();
  SPI.transfer((uint8_t)address & 0x7f);
  delayNanoseconds(50); // Add small delay if sensor needs it (e.g., tSRAD = 100ns)
  uint8_t data = SPI.transfer(0);
  deselect();
  return data;
}
...
void ADNS::writeRegister(const RegisterAddress address, const uint8_t data) {
  ...
  SPI.transfer((uint8_t)address | 0x80);
  SPI.transfer(data);
  delayMicroseconds(1); // Ensure tSCLK-NCS (CS rise time) is met
  deselect();
}
